...
int main( int argc, const char** argv ){
  ...
  while ( true ){    
	...
	cout <<  " FOREGROUND rho A: " << JA.GetBackgroundEstimator()->rho() * CurrentJet.area() << endl;	  
	cout <<  " FOREGROUND Subtracted Jet: " << BackgroundSubtractor( CurrentJet ) << endl;	  
	...
	if (SubtractBg) {
	  CurrentJet = BackgroundSubtractor( CurrentJet );	
	} 
	...
	for (unsigned ijet = 0; ijet < EmbResortedJAResult.size(); ijet++) {
	  ...
	  if ( EmbSubtractBg && EmbBackgroundSubtractor( EmbCurrentJet ).pt() < 1 ) continue;
	  ...
	  if ( sd_jet == 0){
	    ...
	    cout <<  " Subtracted Jet: " << EmbBackgroundSubtractor( EmbCurrentJet ) << endl;	  
	    cout << " --- Skipped. Something caused SoftDrop to return 0 ---" << endl;

	    // Use groomed==original in this case. Not ideal, but probably the best choice.
	    if ( EmbSubtractBg) {
	      EmbCurrentJet = EmbBackgroundSubtractor( EmbCurrentJet );
	    }
