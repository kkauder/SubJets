...
int main( int argc, const char** argv ){
  ...
  while ( true ){    
    ...
    if ( SubjetA.GetLeadingSubjets().size()>0 ){
      LeadingJet    = MakeTLorentzVector( SubjetA.GetJAResult().at(0) );
      if ( SubjetA.GetJAResult().size()>1) {
	SubLeadingJet = MakeTLorentzVector( SubjetA.GetJAResult().at(1) );
      }
	
      for (int i=0; i< SubjetA.GetLeadingSubjets().size(); ++i ){
	new ( LeadingSubjets[i] ) TLorentzVector (MakeTLorentzVector ( SubjetA.GetLeadingSubjets().at(i)));
      }
      for (int i=0; i< SubjetA.GetSubLeadingSubjets().size(); ++i ){
	new ( SubLeadingSubjets[i] ) TLorentzVector (MakeTLorentzVector ( SubjetA.GetSubLeadingSubjets().at(i)));
      }
	...
	// Run analysis
	// ------------
	int Embret = EmbSubjetA.AnalyzeAndFill( particles, 0 );
	...
	if ( EmbSubjetA.GetLeadingSubjets().size()>0 ){
	  EmbLeadingJet    = MakeTLorentzVector( EmbSubjetA.GetJAResult().at(0) );
	  if ( EmbSubjetA.GetJAResult().size()>1) {
	    EmbSubLeadingJet = MakeTLorentzVector( EmbSubjetA.GetJAResult().at(1) );
	  }
	  
	  for (int i=0; i< EmbSubjetA.GetLeadingSubjets().size(); ++i ){
	    TLorentzVector lv = MakeTLorentzVector ( EmbSubjetA.GetLeadingSubjets().at(i));
	    new ( EmbLeadingSubjets[i] ) TLorentzVector (MakeTLorentzVector ( EmbSubjetA.GetLeadingSubjets().at(i)));
	  }
	  for (int i=0; i< EmbSubjetA.GetSubLeadingSubjets().size(); ++i ){
	    TLorentzVector lv = MakeTLorentzVector ( EmbSubjetA.GetSubLeadingSubjets().at(i));
	    new ( EmbSubLeadingSubjets[i] ) TLorentzVector (MakeTLorentzVector ( EmbSubjetA.GetSubLeadingSubjets().at(i)));
	  }
	}
      }
